  ...
  zh: {
    ...
    specs: {
      ...
      faqs: [
        ...
        },
        {
          ...
          a: "完全可以。天御 是完美辅佐高频 DMA 硬件板卡的不二之选。在您通过外部 FPGA 网卡芯片直接透传游戏内存数据的同时，目标计算轨迹可经由局域网流向您的 天御 信号网桥节点执，所有的输入解算与底层逻辑绝不在游戏系统内停留半步。"
        },
        {
          q: "它是否能支持各种自训练的卷积神经网络 (CV) 辅助和视觉模型？",
          a: "当然支持。次级接收主机所接收的信息仅是纯粹的物理像素偏移。您可在分流机或者主控边缘节点部署摄像头或推流画面进行模型推理，得出的实时相对坐标向量经由网络端口分发，天御 会瞬间在硬件级蓝牙上打包投递完毕。"
        },
        {
    ...
    shop: {
      ...
      comparisons: {
        tag: "实体协议外设 vs. 网络软桥接",
        title: "与其他硬件产品的数据较量",
        desc: "KmBox、Makcu 等传统的单片机实体发包控制板不仅需要昂贵的境外运费甚至排队缺货，更伴随着高难度的自定义固件刷写、电烙铁焊接、繁杂的高频排线以及被游戏厂商针对性反制签名物理 PID 的劣势。天御 顺应浪潮，通过创新的内网加密软网桥实现完全革新。",
        metricsHeader: "衡量维度与各模块能力",
        antheCol: "天御 (网桥版软转发方案)",
    ...
    terms: {
      ...
      calloutDesc: "本电子销售契意管辖您购买 天御 数字永久密钥的消费行为。依据英国 2015《消费者权利法案 (CRA 2015)》与 2013《消费者合同规程 (CCR 2013)》，我们将保障关于数字产品内容质量合格、相符性描述以及在数字化退款机制等方面的最低法定权益。",
      ...
      sections: [
        {
          num: "1.",
          title: "我们的运营实体",
          desc: "天御 数字服务系统、安装执行包以及附属配置项（统称“本软件”或“服务”）由 Anthe Digital Systems Ltd.（以下统称“我们”或“本公司”）独立持有并合法运营。"
        },
        {
          num: "2.",
          title: "购买者契约和资质要求",
